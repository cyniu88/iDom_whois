...
static void print_assoclist(const struct iwinfo_ops *iw, const char *ifname , struct a_MAC * my_MAC, int many_mac)
{
           ...
           /*format_signal(e->signal),
            ...
            for (int k =0 ; k < many_mac; ++k)
            {
                ...
                if (strcmp(  my_MAC[k].array_MAC ,format_bssid(e->mac)) == 0)
                {
                    ...
                    {
                        my_MAC[k].state=ON;
                        my_MAC[k].state_now=ON;
                        printf(" jest  %s \n", my_MAC[k].array_MAC);
                    }
                    //
                    my_MAC[k].state_temp=ON;
                ...
                else
                {
                    ...
                    if ( my_MAC[k].state==ON && my_MAC[k].state_now==OFF && my_MAC[k].state_temp==OFF)
                    {
                        my_MAC[k].state=OFF;
...
//        printf(" udało sie zaalokowac  %i \n ",many_mac);
    ...
    do {
        ...
        print_assoclist(iw, "wlan0" , &my_MAC, many_mac);
